...
import { applyDetectionEffect, useAethersDetection, rollDetectionCheck } from '../../scripts/aethers-detection/detection.js';
...
describe("Aether's Detection", () => {
  ...
  describe('applyDetectionEffect', () => {
    ...
    test('Detection effect grants advantage on Investigation checks', async () => {
      await applyDetectionEffect(mockActor, 'basic-refined');

      const createCall = mockActor.createEmbeddedDocuments.mock.calls[0];
      const effectData = createCall[1][0];

      // Should have changes that affect Investigation skill
      expect(effectData.changes).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            key: 'system.skills.inv.bonuses.check',
            mode: 0, // Custom mode in dnd5e
            value: '+1d20' // Advantage = roll 2d20kh1, but bonuses use this format
          })
        ])
      );
    });
  ...
  describe('useAethersDetection', () => {
    ...
    test('returns success with fuel and effect details', async () => {
      const result = await useAethersDetection(mockActor, mockItem, mockAetherFuel);
