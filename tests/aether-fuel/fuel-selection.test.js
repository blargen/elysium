...
describe('Aether Fuel Selection', () => {
      ...
      mockActor.items._items = [
        {
          ...
          flags: { elysium: { isAetherFuel: true, aetherQuality: 'basic-refined' } },
          system: { uses: { value: 5 } },
          getFlag: function(scope, key) { return this.flags[scope]?.[key]; }
        },
        {
          name: 'Regular Potion',
          flags: {},
      ...
      mockActor.items._items = [
        {
          ...
          flags: { elysium: { isAetherFuel: true, aetherQuality: 'unrefined' } },
          system: { uses: { value: 3 } },
          getFlag: function(scope, key) { return this.flags[scope]?.[key]; }
        },
        {
  ...
  describe('getQualityDescription', () => {
    test('returns correct description for unrefined', () => {
      expect(getQualityDescription('unrefined')).toBe('⚠️ TOXIC - Risk of toxicity buildup');
    });

    test('returns correct description for basic-refined', () => {
      expect(getQualityDescription('basic-refined')).toBe('⚪ Neutral - Safe, no bonuses');
    });

    test('returns correct description for rarefied', () => {
      expect(getQualityDescription('rarefied')).toBe('🟢 Enhanced - Provides bonuses');
    });

    test('returns correct description for prometheum', () => {
      expect(getQualityDescription('prometheum')).toBe('🟣 Premium - Significant bonuses');
    });

    test('returns correct description for wild', () => {
      expect(getQualityDescription('wild')).toBe('🌀 Chaotic - Wild Magic effects');
    });
